...
public:
    ...
    ref(T *ptr) : m_ptr(ptr) { if (m_ptr) ((NoriObject *) m_ptr)->incRef(); }

    /// Copy-constructor
    ref(const ref &pRef) : m_ptr(pRef.m_ptr) { if (m_ptr) ((NoriObject*) m_ptr)->incRef(); }

    /// Destroy this reference
    ~ref() { if (m_ptr) ((NoriObject*) m_ptr)->decRef(); }
    ...
    inline ref& operator= (const ref& r) {
        if (m_ptr == r.m_ptr)
            return *this;
        if (m_ptr)
            ((NoriObject*) m_ptr)->decRef();
        m_ptr = r.m_ptr;
        if (m_ptr)
            ((NoriObject*) m_ptr)->incRef();
        return *this;
    }
