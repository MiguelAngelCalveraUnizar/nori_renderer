...
void MemoryStream::resize(size_t size) {
    if (!m_ownsBuffer)
        throw NoriException("Tried to resize a buffer, which doesn't "
...
void MemoryStream::read(void *ptr, size_t size) {
    if (m_pos + size > m_size) {
        ...
        m_pos += sizeRead;
        throw NoriException("Reading over the end of a memory stream ");
    }
...
//NORI_IMPLEMENT_CLASS(MemoryStream, false, Stream)
NORI_NAMESPACE_END
