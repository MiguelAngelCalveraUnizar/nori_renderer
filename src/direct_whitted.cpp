...
public:
		...
		Color3f Li(const Scene* scene, Sampler* sampler, const Ray3f& ray) const {
			...
			//Find the surface that is visibble in the requested direction
			Intersection its;
			...
			//lets iterate over all emitters
			for (unsigned int i = 0; i < lights.size(); ++i)
			{
				...
				// For that, we build a shadow ray (sray), and compute the intersection
