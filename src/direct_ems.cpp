...
protected:
	...
	Color3f Li(const Scene* scene, Sampler* sampler, const Ray3f& ray) const {
		...
		if (its.mesh->getBSDF()->hasDisplacementMap()) {
			...
			//it_eval.shading.n = Frame((Normal3f)(its.shading.dpdu.cross(its.shading.dpdv) + dv * its.shading.dndv).normalized());
			...
			Vector3f new_dpdu = its.shading.dpdu + (uDisplace - displace) / du * its.geoFrame.n + displace * its.shading.dndu;
			Vector3f new_dpdv = its.shading.dpdv + (vDisplace - displace) / dv * its.geoFrame.n + displace * its.shading.dndv;

			its.shading.n = its.geoFrame.n + uDisplace * its.geoFrame.n.cross(its.shading.dpdv)
				- vDisplace * its.geoFrame.n.cross(its.shading.dpdu);
			its.shading.n = its.shading.n.normalized();
